function [output]=probtrack_con_mtx_alx(out_dir,ROI_dir,tract_dir)
%%%alex
%% DOCUMENTATION
...
con_mtx=zeros(length(tract_list),length(roi_list));
...
for i=1:length(tract_list)
    ...
    scale_factor(i)=max(roi_size)./roi_size(i); %for each tract
...
scale_factor=repmat(scale_factor,1,length(roi_list)); % make nxn scale factor array
