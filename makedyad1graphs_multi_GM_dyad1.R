...
#######
extension<-'dyad1_gm'
myylim<- 0.15 # for gm dyad1 , 0.05 for wm in dyad1
...
#mydyad1<-mydyad1[which(mydyad1$resolution == myres & mydyad1$hemi == 'left'),]
mydyad1<-mydyad1[which(mydyad1$hemi == 'left' & mydyad1$fibers == 0),]
...
ggplot(mydyad1, aes(x=directions, y=dispersion, group=ROI, facet_grid(~resolution))) +
  ...
  theme(axis.text.x = element_text(face="bold",  size=14, angle=0),
        ...
        # panel.border = element_blank(),
        panel.background = element_blank())
ggsave(paste(output.path,extension,toString(myres),'allinone_panels.pdf',sep=''), plot = last_plot(), device = 'pdf',
       scale = 1, width = 10, height = 5, units = c("in"),dpi = 300)
...
ggsave(paste(output.path,extension,toString(myres),'3panels.pdf',sep=''), plot = last_plot(), device = 'pdf',
       ...
       #  scale_y_continuous(name="Dyad1 Dispersion", limits=c(0, myylim), breaks=seq(0, myylim, myylim))+
