...
for rr = 1:length(resolutions)
    ...
    %v_size=vox_sizes{rr};
    %percent = percents{rr};
    main_prefix=['/Volumes/TrinityDrive/N51200_v2/bedpost/' ];
    ...
    for aa = 1:length(angles)
        angle=angles{aa}
        for dd = 1:4
            fa_file=[main_prefix '/bedpost_fullbrain_ESR' angle '.bedpostX/dyads' num2str(dd) '_dispersion.nii.gz'];
...
res100_dyad1_L=squeeze(out_array(1,:,1,:,2))'
res100_dyad1_R=squeeze(out_array(1,:,1,:,3))'

res100_dyad2_L=squeeze(out_array(1,:,2,:,2))'
res100_dyad2_R=squeeze(out_array(1,:,2,:,3))'

res100_dyad3_L=squeeze(out_array(1,:,3,:,2))'
res100_dyad3_R=squeeze(out_array(1,:,3,:,3))'

res100_dyad4_L=squeeze(out_array(1,:,4,:,2))'
res100_dyad4_R=squeeze(out_array(1,:,4,:,3))'
